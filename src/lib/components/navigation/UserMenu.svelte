...
<DropdownMenu.Root>
  <DropdownMenu.Trigger class={cn('rg-button',className)} data-variant="ghost" data-size="sm" title={label}>{#if avatar}{@render avatar()}{/if}<span style="display:grid;text-align:left;line-height:1.15"><strong>{name}</strong>{#if detail}<small style="color:var(--rg-fg-subtle);font-weight:500">{detail}</small>{/if}</span></DropdownMenu.Trigger>
