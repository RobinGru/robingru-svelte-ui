...
<div {...rest} class={cn('rg-pin',className)} role="group" aria-label={label}>{#each Array.from({length}) as _,i}<input class="rg-input" type={masked?'password':'text'} inputmode="numeric" autocomplete={i===0?'one-time-code':'off'} maxlength="1" value={value[i]??''} aria-label={`${label} Stelle ${i+1}`} oninput={(e)=>update(i,e)} onkeydown={(e)=>key(i,e)}/>{/each}</div>
