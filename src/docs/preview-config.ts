...
export function previewFor(name: string): PreviewConfig {
  switch (name) {
    ...
    case 'CopyButton': return { props: { value: 'bun add github:RobinGru/robingru-svelte-ui' }, code: importCode(name, '<CopyButton value="bun add github:RobinGru/robingru-svelte-ui" />'), height: 'compact' };
    ...
    case 'SplitButton': return { props: { label: 'Veröffentlichen', items: [{ label: 'Als Entwurf speichern' }, { label: 'Vorschau öffnen' }, { label: 'Veröffentlichung planen', separatorBefore: true }] }, code: importCode(name, '<SplitButton label="Veröffentlichen" {items} />'), height: 'compact' };
    ...
    case 'Calendar': return { props: { value: '2026-08-27', label: 'Veröffentlichungsdatum' }, code: importCode(name, '<Calendar bind:value label="Veröffentlichungsdatum" />'), height: 'tall' };
    ...
    case 'Combobox': return { props: { value: 'team', label: 'Workspace-Typ', options: [{ value: 'solo', label: 'Solo', description: 'Für einzelne Projekte' }, { value: 'team', label: 'Team', description: 'Gemeinsam verwalten' }, { value: 'company', label: 'Unternehmen', description: 'Mehrere Bereiche' }] }, code: importCode(name, '<Combobox bind:value {options} label="Workspace-Typ" />'), height: 'compact' };
    ...
    case 'FormField': return { props: { label: 'Projektname', for: 'demo-project', hint: 'Kann später geändert werden.', required: true, children: raw('<input id="demo-project" class="rg-input" value="Produkt-Dokumentation" />') }, code: importCode(name, '<FormField label="Projektname" required>\n  <Input />\n</FormField>') };
    case 'Input': return { props: { value: 'Produkt-Workspace', placeholder: 'Projektname', 'aria-label': 'Projektname' }, code: importCode(name, '<Input bind:value placeholder="Projektname" />'), height: 'compact' };
    case 'InlineEdit': return { props: { value: 'RobinGru Workspace', label: 'Projektname', required: true, onsave: noop }, code: importCode(name, '<InlineEdit bind:value label="Projektname" />'), height: 'compact' };
    ...
    case 'MentionInput': return { props: { value: 'Bitte mit @mara abstimmen.', label: 'Kommentar', suggestions: [{ value: 'mara', label: 'Mara Klein', description: 'Produktdesign', initials: 'MK' }, { value: 'jonas', label: 'Jonas Weber', description: 'Engineering', initials: 'JW' }] }, code: importCode(name, '<MentionInput bind:value {suggestions} />') };
    case 'MultiSelect': return { props: { values: ['design', 'svelte'], label: 'Themen', options: [{ value: 'design', label: 'Design-System' }, { value: 'svelte', label: 'Svelte 5' }, { value: 'accessibility', label: 'Accessibility' }, { value: 'data', label: 'Data UI' }] }, code: importCode(name, '<MultiSelect bind:values {options} label="Themen" />'), height: 'compact' };
    ...
    case 'TransferList': return { props: { value: ['analytics'], availableLabel: 'Verfügbare Bereiche', selectedLabel: 'Zugewiesen', items: [{ value: 'projects', label: 'Projekte' }, { value: 'analytics', label: 'Analytics' }, { value: 'billing', label: 'Abrechnung' }, { value: 'members', label: 'Mitglieder' }] }, code: importCode(name, '<TransferList bind:value {items} />'), height: 'tall' };

    case 'AppShell': return { props: { contained: false, sidebar: raw(`<aside class="rg-sidebar" style="--sidebar-width:7rem;min-height:12rem"><nav class="rg-sidebar-content"><a class="rg-nav-item" data-active="true" href="${path('/start')}">Start</a><a class="rg-nav-item" href="${path('/dashboard')}">Projekte</a></nav></aside>`), header: raw('<header class="rg-navbar"><strong>Workspace</strong></header>'), children: raw('<div style="padding:1rem"><strong>Hauptinhalt</strong><p style="color:var(--rg-fg-muted)">Ein klares Shell-Raster.</p></div>') }, code: importCode(name, '<AppShell {sidebar} {header}>…</AppShell>'), height: 'tall', padded: false };
    case 'Breadcrumbs': return { props: { items: [{ label: 'Projekte', href: path('/dashboard') }, { label: 'Übersicht', href: path('/components') }, { label: 'Einstellungen', current: true }] }, code: importCode(name, '<Breadcrumbs {items} />'), height: 'compact' };
    case 'FileTree': return { props: { selected: 'button', expanded: ['src', 'components'], nodes: [{ id: 'src', label: 'src', kind: 'folder', children: [{ id: 'components', label: 'components', kind: 'folder', children: [{ id: 'button', label: 'Button.svelte', kind: 'file', modified: 'heute' }, { id: 'input', label: 'Input.svelte', kind: 'file' }] }, { id: 'index', label: 'index.ts', kind: 'file' }] }, { id: 'package', label: 'package.json', kind: 'file' }] }, code: importCode(name, '<FileTree {nodes} bind:selected bind:expanded />'), height: 'tall' };
    ...
    case 'PageHeader': return { props: { eyebrow: 'Workspace', title: 'Projektübersicht', description: 'Status, Aktivitäten und nächste Schritte an einem Ort.', headingLevel: 4, actions: raw('<button class="rg-button">Projekt anlegen</button>') }, code: importCode(name, '<PageHeader title="Projektübersicht" />') };
    case 'Pagination': return { props: { page: 8, total: 24 }, code: importCode(name, '<Pagination page={8} total={24} />'), height: 'compact' };
    case 'ResizablePanels': return { props: { value: 42, first: raw('<div><strong>Navigation</strong><p style="color:var(--rg-fg-muted)">Projektstruktur und Filter</p></div>'), second: raw('<div><strong>Arbeitsbereich</strong><p style="color:var(--rg-fg-muted)">Inhalte bleiben unabhängig scrollbar.</p></div>') }, code: importCode(name, '<ResizablePanels bind:value {first} {second} />'), height: 'tall', padded: false };
    case 'Sidebar': return { props: { width: '13rem', collapsible: true, header: raw('<strong>RobinGru</strong>'), collapsedHeader: raw('<strong aria-hidden="true">R</strong>'), children: raw(`<a class="rg-nav-item" data-active="true" href="${path('/start')}">Übersicht</a><a class="rg-nav-item" href="${path('/components')}">Komponenten</a><a class="rg-nav-item" href="${path('/tokens')}">Tokens</a>`), collapsedChildren: raw(`<a class="rg-nav-item" aria-label="Übersicht" title="Übersicht" href="${path('/start')}">⌂</a><a class="rg-nav-item" aria-label="Komponenten" title="Komponenten" href="${path('/components')}">□</a><a class="rg-nav-item" aria-label="Tokens" title="Tokens" href="${path('/tokens')}">◌</a>`), footer: raw('<small style="color:var(--rg-fg-muted)">v0.3.1</small>') }, code: importCode(name, '<Sidebar collapsible {header} {collapsedHeader}>…</Sidebar>'), height: 'tall', padded: false };
    ...
    case 'TreeView': return { props: { selected: 'components', expanded: ['workspace'], checkable: true, checked: ['tokens'], nodes: [{ id: 'workspace', label: 'Workspace', children: [{ id: 'components', label: 'Komponenten', description: '129 Bausteine' }, { id: 'tokens', label: 'Design Tokens' }] }, { id: 'docs', label: 'Dokumentation' }] }, code: importCode(name, '<TreeView {nodes} bind:selected bind:expanded />'), height: 'tall' };
    ...
    case 'ActivityFeed': return { props: { items: [{ name: 'Mara Klein', initials: 'MK', action: 'hat das Dashboard aktualisiert', target: 'Release-Übersicht', time: 'vor 4 Min.', unread: true, tone: 'primary' }, { name: 'Jonas Weber', initials: 'JW', action: 'hat einen Kommentar ergänzt', detail: 'Bitte die Kennzahlen vor dem Review abstimmen.', time: 'vor 18 Min.', tone: 'info' }, { name: 'Studio Nord', initials: 'SN', action: 'hat Labels ergänzt', target: 'Produkt-Dokumentation', time: 'vor 1 Std.', tags: ['Dokumentation', { label: 'Design', tone: 'primary' }], tone: 'success' }] }, code: importCode(name, '<ActivityFeed {items} />') };
    case 'AuditLog': return { props: { entries: [{ id: '1', actor: 'Mara Klein', action: 'änderte den Status von', target: 'Release 0.4', time: 'vor 4 Min.', tone: 'success', initials: 'MK', detail: 'Von „In Prüfung“ zu „Freigegeben“.' }, { id: '2', actor: 'Jonas Weber', action: 'aktualisierte', target: 'Berechtigungen', time: 'vor 32 Min.', tone: 'primary', initials: 'JW' }, { id: '3', actor: 'Robin Gru', action: 'erstellte', target: 'Workspace', time: 'gestern', initials: 'RG' }] }, code: importCode(name, '<AuditLog {entries} />') };
    case 'ComparisonTable': return { props: { columns: [{ id: 'solo', label: 'Solo' }, { id: 'team', label: 'Team', badge: 'Empfohlen', highlighted: true }, { id: 'business', label: 'Business' }], features: [{ label: 'Projekte', values: { solo: 3, team: 'Unbegrenzt', business: 'Unbegrenzt' } }, { label: 'Teammitglieder', values: { solo: false, team: 12, business: 'Unbegrenzt' } }, { label: 'Analytics', values: { solo: false, team: true, business: true } }, { label: 'Audit Log', values: { solo: false, team: false, business: true } }] }, code: importCode(name, '<ComparisonTable {columns} {features} />'), height: 'tall', padded: false };
    ...
    case 'KanbanBoard': return { props: { columns: [{ id: 'todo', title: 'Offen', cards: [{ id: 'a', title: 'Combobox dokumentieren', description: 'API und Tastaturzustände ergänzen.', tags: ['Docs'], assignee: 'Mara Klein' }] }, { id: 'progress', title: 'In Arbeit', tone: 'primary', limit: 3, cards: [{ id: 'b', title: 'Visual Tests aktualisieren', meta: 'Heute', tags: ['QA'], assignee: 'Jonas Weber' }] }, { id: 'done', title: 'Fertig', tone: 'success', cards: [{ id: 'c', title: 'Token-Skala abstimmen', meta: 'Gestern' }] }] }, code: importCode(name, '<KanbanBoard bind:columns />'), height: 'tall', padded: false };
    ...
    case 'FilterBar': return { props: { title: 'Berichtsfilter', activeCount: 2, onreset: noop, children: raw('<select class="rg-select" aria-label="Segment"><option>Alle Segmente</option><option selected>Team</option></select><input class="rg-input" type="date" value="2026-08-26" aria-label="Stichtag" />') }, code: importCode(name, '<FilterBar activeCount={2}>…</FilterBar>') };
    ...
    case 'ArticleHeader': return { props: { eyebrow: 'Design-System', title: 'Ein ruhiges Design-System', description: 'Eine ruhige Komponentenbasis für Apps, Dashboards und redaktionelle Inhalte.', align: 'start', headingLevel: 4 }, code: importCode(name, '<ArticleHeader title="Ein ruhiges Design-System" />') };
    ...
    case 'Blockquote': return { props: { tone: 'primary', source: 'Designprinzip', children: raw('Gute UI lenkt Aufmerksamkeit, ohne selbst permanent Aufmerksamkeit zu verlangen.') }, code: importCode(name, '<Blockquote source="Designprinzip">…</Blockquote>') };
    ...
    case 'Figure': return { props: { caption: 'Komponenten teilen dieselbe visuelle Grammatik.', credit: 'Design-System', aspect: '16 / 7', media: raw('<div style="height:100%;display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem;padding:1.25rem;background:var(--rg-surface-sunken)"><div class="rg-card" style="display:grid;align-content:start;gap:.35rem;padding:.8rem"><small style="color:var(--rg-fg-muted)">Editorial</small><strong>Artikel</strong><span style="color:var(--rg-fg-muted);font-size:var(--rg-text-xs)">Lesefluss und Kontext</span></div><div class="rg-card" style="display:grid;align-content:start;gap:.35rem;padding:.8rem;background:var(--rg-primary-soft)"><small style="color:var(--rg-fg-muted)">Dashboard</small><strong>84 %</strong><span style="color:var(--rg-fg-muted);font-size:var(--rg-text-xs)">Im Zielbereich</span></div><div class="rg-card" style="display:grid;align-content:start;gap:.35rem;padding:.8rem"><small style="color:var(--rg-fg-muted)">Anwendung</small><strong>24</strong><span style="color:var(--rg-fg-muted);font-size:var(--rg-text-xs)">Aktive Projekte</span></div></div>') }, code: importCode(name, '<Figure {media} caption="…" />') };
    case 'NewsletterCard': return { props: { title: 'Release Notes', description: 'Neue Komponenten und nachvollziehbare Designentscheidungen.', privacyText: 'Kein Tracking, jederzeit abbestellbar.', onsubmit: noop }, code: importCode(name, '<NewsletterCard title="Release Notes" />') };
    ...
    case 'ShareButtons': return { props: { url: 'https://example.com/design-system', title: 'Design-System', platforms: ['copy', 'email', 'linkedin'] }, code: importCode(name, '<ShareButtons {url} {title} />') };
